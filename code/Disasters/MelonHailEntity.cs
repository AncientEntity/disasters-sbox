...
namespace Disasters
{
	public class MelonHailEventEntity : EventEntity
	{
		...
		private void SpawnMelon()
		{
			...
			newMelon.Position = new Vector3(DisastersGame.random.Next(-1227,1601), DisastersGame.random.Next( -1691, 1159 ), 1126 );
			newMelon.Scale *= (float)DisastersGame.random.NextDouble() * 1f + 5f;
			...
			newMelon.PhysicsGroup.Velocity = Vector3.Random * ((float)DisastersGame.random.NextDouble() * 10000f);
